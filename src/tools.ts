import { Player, Rule, Ruleset } from "./types";
/**
 * Extract the Rule ID from a Rule
 */
export type RuleIdFromRule<T> = T extends Rule<any, infer TId> ? TId : never;

/**
 * Filtre un tableau de règles en excluant celles dont l'ID est dans TExclude
 */
export type FilterRules<
  TRules extends Rule<Player, string>[],
  TExclude extends string,
> = TRules extends [
  infer First extends Rule<Player, string>,
  ...infer Rest extends Rule<Player, string>[],
]
 ...
 * @returns
 */
export function makeRuleset<TRules extends Rule<Player, TRulesId>[], TRulesId extends string>(
  name: string,
...
export function extendRuleset<
  TFrom extends Rule<Player, string>[],
  TExcludedIds extends RuleIdFromRule<TFrom[number]>,
  TNewRules extends Rule<Player, TNewRulesIds>[],
